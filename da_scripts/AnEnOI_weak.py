"""
This script runs the whole cycled DA experiment using AnEnOI.
"""
import numpy as np
...
mean_dir = os.environ['SLURM_SCRATCH']+'/an_weak/'
...
for k in range(Nt):
    ...
    # find catalog indices of Ne nearest neighbors, atm only
...
#    d2 += np.tanh(np.mean((pa_mean[np.newaxis,2,::2,::2]-catalogA[:,:,:,4])**2,axis=(1,2))/2.75e5)
    d2 = np.mean((ast_mean[np.newaxis,1::2,::2]-catalogA[:,:,:,0])**2,axis=(1,2))/18 # scaled squared distances
    d2 += np.mean((hmixa_mean[np.newaxis,1::2,::2]-catalogA[:,:,:,1])**2,axis=(1,2))/0.02
    d2 += np.mean((pa_mean[np.newaxis,0,::2,::2]-catalogA[:,:,:,2])**2,axis=(1,2))/1e5
    d2 += np.mean((pa_mean[np.newaxis,1,::2,::2]-catalogA[:,:,:,3])**2,axis=(1,2))/5.3e5
    d2 += np.mean((pa_mean[np.newaxis,2,::2,::2]-catalogA[:,:,:,4])**2,axis=(1,2))/2.75e5
...
#        d2 += np.tanh(np.mean((po_mean[np.newaxis,0,575:959:6,5::6]-catalogS[:,:,1:,1])**2,axis=(1,2))/4.75)
        d2 = np.mean((sst_mean[np.newaxis,576:960:6,0::6]-catalogS[:,:,:,0])**2,axis=(1,2))/0.85
        d2 += np.mean((po_mean[np.newaxis,0,575:959:6,5::6]-catalogS[:,:,1:,1])**2,axis=(1,2))/4.75
